...
| file | bug | status |
|---|---|---|
| `concurrent.qn` / `concurrent_catch.qn` | two tasks calling one extension concurrently desync the connection ("unknown stream id"; a host-reach yield can kill the extension's serve loop) | **FIXED** (fair queuing: concurrent callers park FIFO and are handed the connection in turn; was an in-flight guard → busy error) |
