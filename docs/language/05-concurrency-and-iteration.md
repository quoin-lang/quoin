...
>   - **`WorkerService`** — `WorkerService.host:'unit.qn' class:'Name'` hosts a class in a dedicated isolate and answers a **proxy**: ordinary method sends become RPC into the worker, state stays there, and calls serialize — an actor, effectively. `serviceStop` ends it.
