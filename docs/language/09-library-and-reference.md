...
> - **`mod` (prefix `%`)** — `%'…%{expr}…'` is inline interpolation: the compiler lowers the literal to string concatenation, so each `%{expr}` compiles in the **enclosing scope** — locals, parameters, `self`, leading-dot sends (`%{.name}`), and instance fields (`%{@name}`) all work — and is stringified with `.s`. A malformed fragment is a compile error. Prefix `%` on a *computed* string interpolates reflectively at runtime instead: caller **locals only** (no `self`/`@ivars`), and a malformed fragment raises a catchable `ParseError`.
