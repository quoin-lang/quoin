...
pub fn build_worker_class() -> NativeClassBuilder {
    NativeClassBuilder::new("Worker", Some("Object"))
        ...
        .class_method("spawn:", |vm, mc, receiver, args| {
            ...
            let reg = path.clone();
            wrap_handle(vm, mc, receiver, &reg, "thread", None, spawn_worker(path))
        })
        ...
        .class_method("spawn:backing:", |vm, mc, receiver, args| {
            ...
            match backing.as_str() {
                ...
                "process" => {
                    let reg = path.clone();
                    let (ch, pid) = crate::worker::spawn_worker_process(path, None)
                        .map_err(QuoinError::Other)?;
                    wrap_handle(vm, mc, receiver, &reg, "process", Some(pid), ch)
                }
