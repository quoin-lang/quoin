...
"
    ...
    collect: -> {|collectBlock:Block|
        ...
        l1 = #();
        "* ('+collect>'+lit+'<'+lit.id).puts;
        lit.each:{ |collectItem|
            x = collectBlock.valueWithSelfOrArg:collectItem;

            "* ('s>'+lit+'<'+lit.id).puts;
            "* ('0>'+collectItem+'<'+lit.id).puts;
            "* ('1>'+x+'<'+lit.id).puts;
    ...
    select: -> {|block:Block|
        l2 = #();
        .each:{ |x| (block.valueWithSelfOrArg:x).if:{ l2.add:x } }
        ^l2
    };
    ...
    all?: -> {|block:Block|
        it = self;
        .each:{ |x|
            (block.valueWithSelfOrArg:x).else:{ ^^false };
        };
        ^true
    };
    ...
    flatten -> {
        l4 = #();
        .each:{ |x|
            (x.class==List).if:{ x.flatten.each:{ |y| l4.add:y } }
                          else:{ l4.add:x }
        }
        ^l4
    };
...
Dictionary <-- {
    ...
    contains?: -> { |kvp:KeyValuePair| (.containsKey?:kvp.key) && ((self.at:kvp.key) == kvp.value) }
     ...
     select: -> {|block:Block|
         l = #();
         .each:{ |x| (block.valueWithSelfOrArg:x).if:{ l.add:x } }
         ^l
     };
