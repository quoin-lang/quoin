...
- Overhaul method dispatch with hierarchy-distance scoring, working toward fully unifying native and user methods under one scored multimethod model (the eventual goal: native methods carry type signatures and the hardcoded type-switching inside native fns is extracted into typed variants the scorer routes between).
  ...
  - [ ] **Phase 2 — unify the method-variant representation + typed native methods.** Generalize the variant from `NativeMethodState{ block, next }` to `{ param_types, guard, body: Block | NativeFn, next }` so native methods become chainable scored candidates (this also fixes the pre-existing "Invalid method object in chain" crash when overriding a native method, e.g. `List <-- { count -> {…} }`). Add a typed builder, e.g. `.typed_instance_method("at:", &["Integer"], fn)`; `.instance_method` becomes the untyped case.
  - [ ] **Phase 3 — migrate native fns.** Extract each native fn's internal type-switching into multiple typed variants routed by the scorer. Incremental, per-method.
