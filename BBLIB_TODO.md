...
- Overhaul method dispatch with hierarchy-distance scoring, working toward fully unifying native and user methods under one scored multimethod model (the eventual goal: native methods carry type signatures and the hardcoded type-switching inside native fns is extracted into typed variants the scorer routes between).
  ...
  - [ ] **Phase 2b — typed native methods.** Add `param_types: Option<Vec<Option<String>>>` to the `Native` body (`None` = legacy/untyped → fallback; `Some` = scored by type). `NativeClass` trait + builder gain `.typed_instance_method("at:", &["Integer"], fn)`; `match_score` reads native param_types from the variant. Then native methods are full multimethod participants and the in-fn type-switching (Phase 3) can be extracted out.
