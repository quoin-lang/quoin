...
## Bugs/Odd Behavior
- [ ] **Operator precedence is inverted/wrong for arithmetic.** In the active pest Pratt parser (`src/parser/pest/parser.rs`, the `PrattParser::new()` `.op(...)` chain), the levels are ordered so that `+`/`-` bind *tighter* than `*`/`/`/`%`, and `..` binds tighter than all arithmetic. Verified: `2 + 3 * 4 == 20` (parses `(2 + 3) * 4`), `2 * 3 + 4 == 14`, and `2 .. 3 + 1` errors (parses `(2..3) + 1`). Comparison is looser than arithmetic; all infix ops are left-associative. Fix the precedence ordering to the conventional one (`*` `/` `%` tighter than `+` `-`, range and comparison placed sensibly) **after** the language-semantics doc is generated — the doc will describe current behavior and carry a callout pointing here.
- [ ] **`-->` (and `->`) don't actually override a same-signature method.** Both operators append a variant to the selector's multimethod chain (`src/vm.rs`, the method-def handling around `DefineMethod`/`OverrideMethod` and the candidate collection in `lookup_method_in_class_hierarchy`). For variants of *equal* specificity, dispatch currently picks the **first-defined**, so redefining a method with the same signature — even via `-->` — is dead code and override silently fails. Verified: `Foo <- { bar -> { 1 } }; Foo <-- { bar --> { 2 } }; Foo.new.bar` returns `1`. Adding a variant is correct (it's needed for multi-dispatch), but the dispatcher should resolve equal-specificity ties in **reverse definition order** (most-recently-defined wins) so that `-->` overrides. Keep `-->`'s requirement that the selector already exist; type/guard-differentiated variants are unaffected. The language doc (`docs/language/03-objects.md` §10/§13) documents the current behavior with a callout pointing here.
